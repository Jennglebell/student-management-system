...
import com.example.studentsmanagement.exceptions.InvalidUniversityClasException;
...
public class UniversityClassService {
    ...
    public UniversityClass addClass(UniversityClass universityClass) {
        ...
        if (universityClass.getYear() < currentYear) {
            throw new InvalidUniversityClasException("Cannot add class in the past!");
        }

        if (universityClass.getYear() > currentYear + 1) {
            throw new InvalidUniversityClasException("Cannot add class in the future!");
        }
